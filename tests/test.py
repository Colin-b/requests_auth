...
class OktaTest(unittest.TestCase):
    ...
    def test_corresponding_oauth2_authorization_code_flow_instance(self):
        okta = requests_auth.OktaAuthorizationCode(
            'testserver.okta-emea.com',
            '54239d18-c68c-4c47-8bdd-ce71ea1d50cd'
        )
        self.assertRegex(okta.code_grant_details.url,
                         ...
                         '&redirect_uri=http%3A%2F%2Flocalhost%3A5000%2F'.replace('?', '\?').replace('+', '\+'))
