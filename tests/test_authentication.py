...
def can_connect_to_server(port: int):
    try:
        response = urllib.request.urlopen('http://localhost:{0}/status'.format(port), timeout=0.5)
...
def test_oauth2_implicit_flow_token_is_not_reused_if_a_url_parameter_is_changing(authenticated_service, token_cache, responses: RequestsMock):
...
def test_oauth2_implicit_flow_token_is_reused_if_only_nonce_differs(authenticated_service, token_cache, responses: RequestsMock):
    auth1 = requests_auth.OAuth2Implicit(
...
def test_oauth2_implicit_flow_token_can_be_requested_on_a_custom_server_port(authenticated_service, token_cache, responses: RequestsMock):
...
def test_oauth2_implicit_flow_post_token_is_sent_in_authorization_header_by_default(authenticated_service, token_cache, responses: RequestsMock):
    auth = requests_auth.OAuth2Implicit(
        TEST_SERVICE_HOST + '/provide_token_as_access_token',
        timeout=TIMEOUT
    )
...
def test_oauth2_implicit_flow_get_token_is_sent_in_authorization_header_by_default(authenticated_service, token_cache, responses: RequestsMock):
    auth = requests_auth.OAuth2Implicit(
...
def test_oauth2_implicit_flow_token_is_sent_in_requested_field(authenticated_service, token_cache, responses: RequestsMock):
    auth = requests_auth.OAuth2Implicit(
        TEST_SERVICE_HOST + '/provide_token_as_access_token',
        timeout=TIMEOUT,
        header_name='Bearer',
...
def test_oauth2_implicit_flow_can_send_a_custom_response_type_and_expects_token_to_be_received_with_this_name(authenticated_service, token_cache, responses: RequestsMock):
    auth = requests_auth.OAuth2Implicit(
...
def test_oauth2_implicit_flow_expects_token_in_id_token_if_response_type_is_id_token(authenticated_service, token_cache, responses: RequestsMock):
    auth = requests_auth.OAuth2Implicit(
...
def test_oauth2_implicit_flow_expects_token_in_id_token_if_response_type_in_url_is_id_token(authenticated_service, token_cache, responses: RequestsMock):
    auth = requests_auth.OAuth2Implicit(
...
def test_oauth2_implicit_flow_expects_token_to_be_stored_in_access_token_by_default(authenticated_service, token_cache, responses: RequestsMock):
    auth = requests_auth.OAuth2Implicit(
        TEST_SERVICE_HOST + '/provide_token_as_access_token',
        timeout=TIMEOUT
    )
...
def test_oauth2_implicit_flow_token_is_reused_if_not_expired(authenticated_service, token_cache, responses: RequestsMock):
    auth1 = requests_auth.OAuth2Implicit(
        TEST_SERVICE_HOST + '/provide_token_as_access_token',
        timeout=TIMEOUT
    )
...
def test_oauth2_implicit_flow_post_failure_if_token_is_not_provided(authenticated_service, token_cache):
    with pytest.raises(Exception) as exception_info:
...
def test_oauth2_implicit_flow_get_failure_if_token_is_not_provided(authenticated_service, token_cache):
    with pytest.raises(Exception) as exception_info:
        call(requests_auth.OAuth2Implicit(
...
def test_oauth2_implicit_flow_post_failure_if_state_is_not_provided(authenticated_service, token_cache):
    with pytest.raises(Exception) as exception_info:
        call(requests_auth.OAuth2Implicit(
            TEST_SERVICE_HOST + '/provide_token_as_access_token_but_without_providing_state',
            timeout=TIMEOUT
        ),)
    assert re.match("state not provided within {'access_token': \['.*'\]}.", str(exception_info.value))


def test_oauth2_implicit_flow_get_failure_if_state_is_not_provided(authenticated_service, token_cache):
    with pytest.raises(Exception) as exception_info:
        call(requests_auth.OAuth2Implicit(
...
def test_oauth2_implicit_flow_failure_if_token_is_not_received_within_the_timeout_interval(authenticated_service, token_cache):
    with pytest.raises(Exception) as exception_info:
        call(requests_auth.OAuth2Implicit(
            TEST_SERVICE_HOST + '/do_not_redirect',
            timeout=TIMEOUT
        ))
    assert str(exception_info.value) == 'User authentication was not received within {timeout} seconds.'.format(timeout=TIMEOUT)


def test_oauth2_implicit_flow_token_is_requested_again_if_expired(authenticated_service, token_cache, responses: RequestsMock):
...
def test_oauth2_authorization_code_flow_get_code_is_sent_in_authorization_header_by_default(authenticated_service, token_cache, responses: RequestsMock):
    auth = requests_auth.OAuth2AuthorizationCode(
        TEST_SERVICE_HOST + '/provide_code_as_anchor_code',
        "http://provide_access_token",
        timeout=TIMEOUT
    )
    responses.add(responses.POST, "http://provide_access_token", json={
...
def test_oauth2_password_credentials_flow_token_is_sent_in_authorization_header_by_default(authenticated_service, token_cache, responses: RequestsMock):
...
def test_oauth2_client_credentials_flow_token_is_sent_in_authorization_header_by_default(authenticated_service, token_cache, responses: RequestsMock):
...
def test_okta_client_credentials_flow_token_is_sent_in_authorization_header_by_default(authenticated_service, token_cache, responses: RequestsMock):
    ...
    responses.add(responses.POST, "https://test_okta/oauth2/default/v1/token", json={
       "access_token": "2YotnFZFEjr1zCsicMWpAA",
       "token_type": "example",
       "expires_in": 3600,
       "refresh_token": "tGzv3JOkF0XG5Qx2TlKWIA",
...
def test_okta_client_credentials_flow_token_is_sent_in_authorization_header_by_default_using_helper(authenticated_service, token_cache, responses: RequestsMock):
...
def test_basic_authentication_send_authorization_header(responses: RequestsMock):
    ...
    assert get_header(responses, auth).get('Authorization') == 'Basic dGVzdF91c2VyOnRlc3RfcHdk'
...
def test_basic_and_api_key_authentication_can_be_combined_deprecated(responses: RequestsMock):
    basic_auth = requests_auth.Basic('test_user', 'test_pwd')
    api_key_auth = requests_auth.HeaderApiKey('my_provided_api_key')
